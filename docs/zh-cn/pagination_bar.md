## 简介
`CPaginationBar` 是一个基于 `PySide2` 的分页控件，用于在应用程序中实现分页功能。它支持扁平样式和普通样式，并且可以显示统计信息和跳转功能。
...
********
## 方法
 - `def setInfos(self, text):` 设置统计信息文字。
 - `def getCurrentPage(self):` 获取当前页。
 - `def setCurrentPage(self, currentPage):` 设置当前页。
 - `def setTotalPages(self, totalPages):` 设置总页数。
 - `def setJumpWidget(self, visible):` 开启跳转控件。
******
## 信号
 - `pageChanged` 当前页发生变化时触发。
******
